# BeamScan — Simulation & Automation Plan

## For the BL4S 2026 team in Córdoba, Argentina

---

## 1. Overview: What This System Does

The BeamScan simulation system turns a simple text file into physics predictions and plots — automatically, in the cloud, with no software installation required.

```
STUDENT                           GITHUB                        RESULT
━━━━━━━━                          ━━━━━━                        ━━━━━━
                                  ┌────────────────-──┐
 Edit YAML file ──── git push ───►│ GitHub Actions    │
 in requests/                     │                   │
                                  │ 1. Parse YAML     │───► predictions.csv
                                  │ 2. Highland calc  │───► distributions.png
                                  │ 3. Plot generator │───► classification.png
                                  │ 4. PR commenter   │───► SUMMARY.md
                                  └──────────────-────┘          │
                                                                 │
 See results ◄──── PR comment ────────────────────────────────-──┘
 on GitHub         with plots
```

### Two Tiers

| Tier | What runs | Speed | Accuracy | Who triggers it |
|------|-----------|-------|----------|-----------------|
| **Highland** (Tier 1) | Python analytical formula | ~30 seconds | Gaussian core only (no tails) | Automatic on every push |
| **Geant4** (Tier 2) | Full Monte Carlo in Docker | ~5–15 min | Full physics (Molière tails, δ-rays, energy loss) | Manual trigger by teacher |

**Students interact exclusively with Tier 1** (fast, instant feedback). Tier 2 is for final validation before the proposal deadline.

---

## 2. Repository Structure

```
bl4s-beamscan/
│
├── .github/workflows/
│   ├── highland-predict.yml     ← AUTO: runs on every push to requests/
│   └── geant4-simulate.yml      ← MANUAL: triggered from Actions tab
│
├── requests/                     ← 👈 STUDENTS WORK HERE
│   ├── examples/                 ← Read-only reference files
│   │   ├── template.yaml         ← Copy this to start
│   │   ├── valentina_pvc_detection.yaml
│   │   ├── tomas_thickness_study.yaml
│   │   └── lucia_heritage_study.yaml
│   └── full_classification.yaml  ← The "official" 11-material run
│
├── results/                      ← 📊 AUTO-GENERATED by CI
│   ├── full_classification/      ← One folder per request
│   │   ├── predictions.csv
│   │   ├── distributions.png
│   │   ├── classification.png
│   │   └── SUMMARY.md
│   └── geant4/                   ← Monte Carlo event-level data
│
├── simulation/                   ← Geant4 C++ source code
│   ├── CMakeLists.txt
│   ├── beamscan.cc
│   ├── src/                      ← Physics implementation
│   ├── include/                  ← Header files
│   └── macros/                   ← Geant4 run macros (auto/ for generated)
│
├── analysis/                     ← Python analysis engine
│   ├── highland_calculator.py    ← Core: reads YAML → produces plots
│   └── analyze_geant4.py         ← Compares Geant4 output with Highland
│
├── scripts/                      ← Helper utilities
│   ├── generate_macros.py        ← Converts YAML → Geant4 .mac files
│   └── pseudo_mc.py              ← Gaussian event generator for CI testing
│
├── data/
│   └── materials_database.json   ← Reference: X₀, ρ, θ₀ for all materials
│
├── docs/guides/
│   └── STUDENT_GUIDE.md          ← Bilingual (Spanish/English) instructions
│
├── .gitignore
└── README.md                     ← Landing page with quick-start + physics
```

---

## 3. How Each Workflow Works

### Workflow 1: Highland Prediction (automatic)

**Trigger:** Any push or PR that touches `requests/*.yaml`

```
┌─────────────────────────────────────────────────────────────────┐
│ highland-predict.yml                                            │
│                                                                 │
│  Job 1: detect-requests                                         │
│    └─ git diff → find changed YAML files → build matrix         │
│                                                                 │
│  Job 2: predict (parallel, one per request file)                │
│    ├─ pip install numpy matplotlib pyyaml                       │
│    ├─ python highland_calculator.py <request.yaml>              │
│    │   ├─ Parse YAML → list of (material, momentum) pairs       │
│    │   ├─ Highland formula: θ₀ = (13.6/p)√(x/X₀)[1+0.038ln()]   │
│    │   ├─ Generate: distributions.png                           │
│    │   ├─ Generate: classification.png                          │
│    │   ├─ Generate: predictions.csv                             │
│    │   └─ Generate: SUMMARY.md (with discrimination matrix)     │
│    ├─ Upload results as GitHub artifact                         │
│    └─ Post SUMMARY.md as PR comment (if triggered by PR)        │
│                                                                 │
│  Job 3: commit-results (on main branch pushes only)             │
│    └─ Download artifacts → commit to results/ → push            │
└─────────────────────────────────────────────────────────────────┘
```

**What students see:**
- Push a YAML file → 30 seconds later → results appear in PR comment
- Classification plot, distribution overlay, discrimination power matrix
- All without installing anything

### Workflow 2: Geant4 Monte Carlo (manual)

**Trigger:** Manual dispatch from Actions tab (teacher clicks "Run workflow")

```
┌─────────────────────────────────────────────────────────────────┐
│ geant4-simulate.yml                                             │
│                                                                 │
│  Step 1: generate_macros.py                                     │
│    └─ Parse YAML → one .mac file per (material × momentum)      │
│                                                                 │
│  Step 2: Build Geant4 (cached)                                  │
│    └─ cmake + make inside Docker container                      │
│       (or skip if build cache hit)                              │
│                                                                 │
│  Step 3: Run simulations                                        │
│    ├─ Production: ./beamscan -m <macro.mac> for each config     │
│    └─ CI fallback: pseudo_mc.py (Highland + Gaussian sampling)  │
│       Output: events.csv per configuration                      │
│                                                                 │
│  Step 4: Highland prediction (for comparison)                   │
│    └─ Same highland_calculator.py as Tier 1                     │
│                                                                 │
│  Step 5: Compare Geant4 vs Highland                             │
│    ├─ analyze_geant4.py: fit distributions, extract σ           │
│    ├─ Bar chart: Geant4 σ_core vs Highland θ₀                   │
│    ├─ Overlay: actual Geant4 histograms                         │
│    └─ COMPARISON.md with ratio table                            │
│                                                                 │
│  Step 6: Upload artifacts (60 day retention)                    │
└─────────────────────────────────────────────────────────────────┘
```

**When Geant4 is NOT actually installed** (GitHub free tier): the workflow falls back to `pseudo_mc.py` which generates Gaussian-sampled events. This lets the full analysis pipeline run and verifies everything works. The comparison will show ratio ~1.0 (since pseudo-MC is Highland-based).

**When Geant4 IS installed** (Docker image or self-hosted runner): the comparison becomes genuinely interesting — Geant4 includes Molière scattering tails, δ-ray production, and energy loss fluctuations that Highland doesn't. Finding σ_Geant4/θ₀_Highland ≠ 1 is itself a research result.

---

## 4. The Student Experience (Step by Step)

### Day 1: First contact
1. Teacher forks the repo to the team's GitHub organization
2. Each student clones the fork
3. Each student copies `requests/examples/template.yaml` → `requests/myname.yaml`
4. Edits: picks 2–3 materials, sets a momentum
5. Pushes → sees Actions run → sees results in 30 seconds

### Day 2–3: Exploration
- Students try different material combinations, thicknesses, momenta
- Each student's request file is their own "mini-experiment"
- PR comments accumulate results → team discussion
- Students discover: PE ≈ PP (can't separate!), PVC stands out, minerals scatter more

### Day 4–5: Analysis
- Students compare each other's results
- "My 3 GeV run separates PVC better than your 6 GeV run" → discuss 1/p dependence
- Build the full 11-material classification plot together
- Teacher triggers Geant4 run for final validation

### Day 6–7: Proposal integration
- Pick the best plots for the proposal
- Write figure captions using the auto-generated SUMMARY.md numbers
- Link the GitHub repo in the proposal ("public, reproducible, anyone can verify")

---

## 5. Geant4 Docker Setup (For Teacher / CI Admin)

### Option A: GitHub self-hosted runner with Geant4

If you have access to a machine with Geant4 installed:

```bash
# On your machine (Linux, macOS, or WSL)
# Install GitHub Actions runner: https://docs.github.com/en/actions/hosting-your-own-runners

# Install Geant4 11.x
# Build the simulation
cd simulation && mkdir build && cd build
cmake .. -DGeant4_DIR=/path/to/geant4/lib/cmake/Geant4
make -j$(nproc)

# Register as self-hosted runner
# Then in the workflow YAML, change runs-on: ubuntu-latest → runs-on: self-hosted
```

### Option B: Docker on GitHub Actions

Add to the Geant4 workflow:
```yaml
jobs:
  simulate:
    runs-on: ubuntu-latest
    container:
      image: geant4/geant4:11.2.2
```

Note: Geant4 Docker images are large (~2 GB). GitHub Actions has a 14 GB disk limit, so this works but pulls take ~3 minutes.

### Option C: Run locally, upload results

Students or teacher runs Geant4 on their own machine:
```bash
cd simulation/build
./beamscan -m ../macros/auto/PE_3.0GeV_10.0mm.mac
# Copy events.csv to results/geant4/ and push
```

**Recommendation: Start with Option C (no infra needed), move to Option B when comfortable.**

---

## 6. What Gets Generated (Output Reference)

### From Highland Calculator

| File | Contents | Format |
|------|----------|--------|
| `predictions.csv` | θ₀, ΔE, X₀ for each (material, momentum) | CSV |
| `distributions.png` | Overlaid Gaussian scattering curves | PNG, 200 dpi |
| `classification.png` | 2D fingerprint plot (θ₀ at p₁ vs θ₀ at p₂) | PNG, 200 dpi |
| `SUMMARY.md` | Full report with discrimination matrix | Markdown |

### From Geant4 Simulation

| File | Contents | Format |
|------|----------|--------|
| `events.csv` | Per-event: θx, θy, θ3d (one file per config) | CSV |
| `geant4_vs_highland.png` | σ comparison bar chart + distribution overlay | PNG, 200 dpi |
| `COMPARISON.md` | σ_Geant4 / θ₀_Highland ratio table | Markdown |

### From Macro Generator

| File | Contents | Format |
|------|----------|--------|
| `<material>_<p>GeV_<x>mm.mac` | Geant4 run macro | Text |
| `run_all.sh` | Shell script to run all macros sequentially | Bash |

---

## 7. Implementation Checklist

### Phase 1: Now → Feb 23 (repo setup)
- [ ] Create GitHub organization for the team
- [ ] Push this repo structure
- [ ] Verify Highland workflow runs on first push
- [ ] Each student does their first request file
- [ ] Celebrate when first plots appear automatically

### Phase 2: Feb 24 → Mar 2 (exploration)
- [ ] Students submit 3–5 investigation requests each
- [ ] Run the full_classification.yaml for proposal figures
- [ ] Attempt local Geant4 build (teacher or advanced student)
- [ ] If Geant4 works: run one material, compare with Highland

### Phase 3: Mar 3 → Mar 9 (validation)
- [ ] Trigger Geant4 workflow for the full 11-material set
- [ ] Compare Geant4 vs Highland (look for tail effects)
- [ ] Select best plots for proposal
- [ ] Film 1-minute video showing the repo + automated pipeline

### Phase 4: Mar 10 → Mar 13 (submission)
- [ ] Final proposal text (already written, ~1000 words)
- [ ] Embed chosen figures in DOCX
- [ ] Link GitHub repo in proposal
- [ ] Submit before March 13, 23:59 CEST

---

## 8. Why This Architecture Wins Points

### For the "Scientific Method" criterion
- Predictions BEFORE measurement (Highland → Geant4 → real data)
- Every prediction is versioned, timestamped, reproducible
- Students can explain exactly what they expect to see

### For the "Creativity" criterion
- No other BL4S team will have automated CI/CD for their simulation
- Public GitHub repo = open science (CERN values this)
- Bilingual documentation = inclusivity

### For the "Motivation to Participate" criterion
- Students are ALREADY running simulations before the experiment
- Each student's request file shows individual engagement
- The PR comment trail is a visible record of collaborative science

### For the "Feasibility" criterion
- Highland predictions demonstrate the measurement works
- Geant4 validation adds another layer of confidence
- The discrimination matrix quantifies exactly how many events are needed

---

## 9. Troubleshooting & FAQ

**Q: A student's YAML file breaks the workflow.**
A: GitHub Actions only runs on changed files. Other students' results are not affected. Fix the YAML (usually an indentation error) and re-push.

**Q: Can we add custom materials not in the database?**
A: Yes — use any Geant4 NIST material name. The Highland calculator will warn "Unknown material" but Geant4 will handle it. Add the X₀ value to `MATERIALS_DB` in `highland_calculator.py` for analytical predictions.

**Q: The pseudo-MC comparison shows ratio = 1.0 for everything.**
A: Expected — pseudo-MC uses the Highland formula, so it matches by construction. The ratio becomes interesting only with real Geant4 (where Molière tails make σ_core < θ₀_Highland).

**Q: How do we get real Geant4 running?**
A: Easiest path: install Geant4 on one machine (teacher's laptop, university server, or a VM). Build the simulation locally. Run macros manually. Upload results to the repo. The analysis pipeline handles the rest.

**Q: Can we run this on a phone/Chromebook?**
A: The Highland predictions run entirely in GitHub's cloud — you only need a web browser to edit YAML and view results. No local computation needed.
